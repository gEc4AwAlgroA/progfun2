...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    if (b.legalNeighbors.isEmpty) Stream.empty
    else {
      (for {
        nextBlock <- b.legalNeighbors
      } yield nextBlock match { case (b, m) => (b, m :: history) }
      ).toStream
    }
  }
  ...
  lazy val solution: List[Move] = {
    if (!pathsToGoal.isEmpty) {
      val shortestLength = (pathsToGoal.toList map (_._2.length)).min
      (pathsToGoal filter { case (b, ms) => ms.length == shortestLength }).head._2.reverse
    }
    else List()
  }
